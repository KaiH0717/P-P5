...
namespace
{
	D3D11DebugRenderer debugRenderer;
	...
	_In_ int       nCmdShow)
{
	...
	while (true)
	{
		...
		else
		{
			...
			{
				debugRenderer.D3D11Renderer::Update();
				debugRenderer.D3D11Renderer::Render();
				debugRenderer.D3D11Renderer::EndFrame();
				//renderer.Update();
				//renderer.Render();
				//renderer.EndFrame();
			}
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	debugRenderer.SetWindowHandle(hWnd);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
	{
		if (wParam == VK_BACK)
			useDebug = !useDebug;
		swprintf_s(msg, L"WM_KEYDOWN: 0x%llx\n", wParam);
		std::wcout << msg;
	}
	break;
	case WM_KEYUP:
	{
		swprintf_s(msg, L"WM_KEYUP: 0x%llx\n", wParam);
